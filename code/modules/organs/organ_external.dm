...
/obj/item/organ/external/install(var/mob/living/carbon/human/H)
	if(..(H)) return 1
	owner.organs |= src
...
/obj/item/organ/external/removed(var/mob/living/user, var/redraw_mob = 1)
	if(!owner)
		return
...
/obj/item/organ/external/proc/robo_repair(var/repair_amount, var/damage_type, var/damage_desc, obj/item/tool, mob/living/user)
	...
	if(user == src.owner)
		...
		if(grasp)
			user << "<span class='warning'>You can't reach your [src.name] while holding [tool] in it.</span>"
			//your [owner.get_bodypart_name(grasp)].</span>"
			return 0
...
/obj/item/organ/external/proc/fracture()
	...
	// hardsuit rewrite.
	if(owner && !(status & ORGAN_SPLINTED))
