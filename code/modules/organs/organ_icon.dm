...
/obj/item/organ/external/head/get_icon()
	...
	if(owner.should_have_organ(O_EYES))
		...
		if(eyes)
			...
			eyes_icon = new/icon(species.icobase, "eyes[owner.body_build.index]")
			eyes_icon.Blend(rgb(128,0,0), ICON_ADD)

		mob_icon.Blend(eyes_icon, ICON_OVERLAY)

	if(owner.lip_color && (owner.species.flags & HAS_LIPS))
		var/icon/lip_icon = new/icon(species.get_icobase(), "lips[owner.body_build.index]")
	...
	if(force_icon)
		...
		if(!dna)
			...
			if(skeletal)
				...
				mob_icon = new /icon(species.get_icobase(status & ORGAN_MUTATED), icon_state)
